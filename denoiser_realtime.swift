//
//  denoiser_realtime.swift
//  Denoiser Swift
//
//  Created by Marius Verdier on 15/08/2024.
//
...
public class DenoiserGraph: MPSGraph {
    ...
    private func fileLoad(_ file: String, shape: [NSNumber], name: String? = nil) -> MPSGraphTensor {
        let modelData = Denoiser.loadedData[file]!
...
open class Denoiser : NSObject {
    ...
    public static var loadedData: [String: Data] = [:]
    private var denoiserGraph : DenoiserGraph?
    
    public init(chin: Int = 1, chout: Int = 1, growth: Int = 2, depth: Int = 5, inputSize: Int) {
        self.chin = chin
        self.chout = chout
        self.growth = growth
        self.depth = depth
        ...
        self.denoiserGraph = DenoiserGraph(device: graphDevice, queue: commandQueue, frameCount: inputSize, channelCount: 1)
    }
    
    private func loadEncoderData(path: String) -> Void {
        let encConvW = Bundle.main.url(forResource: "\(path).0.weight", withExtension: "bin")
        let encConvWD = try! Data(contentsOf: encConvW!)
        Denoiser.loadedData.updateValue(encConvWD, forKey: "\(path).0.weight")
        let encConvB = Bundle.main.url(forResource: "\(path).0.bias", withExtension: "bin")
        let encConvBD = try! Data(contentsOf: encConvB!)
        Denoiser.loadedData.updateValue(encConvBD, forKey: "\(path).0.bias")
        let encConv2W = Bundle.main.url(forResource: "\(path).2.weight", withExtension: "bin")
        let encConv2WD = try! Data(contentsOf: encConv2W!)
        Denoiser.loadedData.updateValue(encConv2WD, forKey: "\(path).2.weight")
        let encConv2B = Bundle.main.url(forResource: "\(path).2.bias", withExtension: "bin")
        let encConv2BD = try! Data(contentsOf: encConv2B!)
        Denoiser.loadedData.updateValue(encConv2BD, forKey: "\(path).2.bias")
    }
    
    private func loadDecoderData(path: String) -> Void {
        let encConvW = Bundle.main.url(forResource: "\(path).0.weight", withExtension: "bin")
        let encConvWD = try! Data(contentsOf: encConvW!)
        Denoiser.loadedData.updateValue(encConvWD, forKey: "\(path).0.weight")
        let encConvB = Bundle.main.url(forResource: "\(path).0.bias", withExtension: "bin")
    ...
    private func loadLSTMData(path: String) -> Void {
        let lstmKeys = [
            "weight_ih_l0",
            "weight_hh_l0",
            "bias_ih_l0",
        ...
        for k in lstmKeys {
            let enc = Bundle.main.url(forResource: "\(path).\(k)", withExtension: "bin")
            let encD = try! Data(contentsOf: enc!)
            Denoiser.loadedData["\(path).\(k)"] = encD
        }
    }
    
    private func loadData() -> Void {
        for i in 0..<depth {
            loadEncoderData(path: "encoder.\(i)")
            loadDecoderData(path: "decoder.\(i)")
        }
        loadLSTMData(path: "lstm.lstm")
    }
    ...
    public func analyze(input: Data) -> [Float] {
        let result = denoiserGraph!.run(i_data: input)!.signal
        return result
    }
    
    public func analyze(inputBuffer: AVAudioPCMBuffer) -> [Float] {
        let bufferData = inputBuffer.data()
        let result = denoiserGraph!.run(i_data: bufferData)!.signal
